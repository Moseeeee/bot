...
@bot.message_handler(func=lambda message: re.search(r'\b(команды|Команды|сап команды|/help|.Команды|.команды|/commands@sapcmbot|/commands)\b', message.text, re.IGNORECASE) is not None)
...
@bot.message_handler(func=lambda message: message.text.startswith('+хелп'))
...
def handle_messages(message):
        ...
        if chat_member.status in ['administrator', 'creator'] and message.text.startswith('.дел '):
        	reply_message = bot.reply_to(message, "Удаляю сообщения...")
        delete_messages(message, reply_message)
